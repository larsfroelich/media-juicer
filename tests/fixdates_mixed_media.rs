...
fn fixdates_aggregates_failures_for_mixed_media_and_reports_full_progress() {
    ...
    let plan = build_processing_plan(&source_root, Mode::Fixdates, None).expect("plan should build");
    ...
    let ExecutionError::FileFailures(summary) = result.expect_err("should aggregate failures") else {
    ...
    let failure_paths: Vec<_> = summary
        .failures
        .iter()
        .map(|failure| failure.path.file_name().unwrap().to_string_lossy().to_string())
        .collect();
